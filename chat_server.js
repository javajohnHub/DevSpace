...
io.sockets.on("connection", function(socket) {
  ...
  socket.on("joinserver", function(data) {
    ...
    if (exists) {
      ...
      io.sockets.emit("update", {
        username: "Admin",
        text: people[socket.id].name + " is online."
      });
        ...
        for (var i = 0; i < keys.length; i++) {
          if (people[keys[i]].name === whisperTo) {
            ...
            if (socket.id === whisperId) {
              ...
              socket.emit("update", {
                username: "Admin",
                text: "You can't whisper to yourself."
              });
            }
            break;
          }
      ...
      if (io.sockets.adapter.rooms[socket.room] != undefined) {
        ...
        if (_.size(chatHistory[socket.room]) > 10) {
          ...
          if (chatHistory[socket.room] != undefined) {
            chatHistory[socket.room].push(
              people[socket.id].name + ": " + sanitize.escape(msg)
            );
          }
        }
      } else {
  ...
  socket.on("removeRoom", function(id) {
    ...
    if (socket.id === room.owner) {
      ...
      socket.emit("update", {
        username: "Admin",
        text: "Only the owner can remove a room."
      });
    }
  });
  ...
  socket.on("joinRoom", function(id) {
    if (typeof people[socket.id] !== "undefined") {
      ...
      socket.emit("update", {
        username: "Admin",
        text: "Please enter a valid name first."
      });
    }
  });
    ...
    if (people[s.id].inroom) {
      ...
      if (s.id === room.owner) {
        ...
        if (action === "disconnect") {
          io.sockets.emit("update", {
            username: "Admin",
            text: people[s.id].name + " has disconnected from the server."
          });
          if (_.contains(room.people, s.id)) {
          ...
          if (_.contains(room.people, s.id)) {
            ...
            io.sockets.emit("update", {
              username: "Admin",
              text: people[s.id].name + " has left the room."
            });
            s.leave(room.name);
          }
        }
  ...
  socket.on("mouse movement", function(mouse) {
    ...
    socket.broadcast
      .to(socket.room)
      .emit("mouse update", { id: socket.number, pos: mouse.pos });
  });
});
